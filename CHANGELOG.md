...
- Nested subcrate `aauth-hardware-keys/aauth-macos-se-helper`: builds and adhoc-codesigns the `se-helper` CLI into `target/{debug,release}/se-helper` and exposes a typed client (`generate`, `sign_hash`, `list`, `public_key`, `delete`, `is_available`).
...
- Workspace crate `aauth-hardware-keys`: YubiKey PIV and macOS Secure Enclave as a Rust `rlib` (from packages-js/hardware-keys). SE uses a Cargo-built codesigned `se-helper` subprocess (keychain `com.aauth.secure-enclave`) via `aauth-macos-se-helper` — not in-process Swift FFI; not the unused stock SecKey path.
...
- `StaticKeyMaterialProvider::new` to wrap arbitrary `SigningMaterial` (e.g. from `@aauth/bootstrap token`).
